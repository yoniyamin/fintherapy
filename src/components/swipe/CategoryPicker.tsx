...
export default function CategoryPicker({ open, onSelect, onClose, categories }: CategoryPickerProps) {
  ...
  return createPortal(
    <AnimatePresence>
      {open && <CategoryPickerInner cats={cats} onSelect={onSelect} onClose={onClose} />}
    </AnimatePresence>,
    document.body,
  )
}
  ...
  return (
    <>
      ...
      >
        ...
        <h3 id="category-picker-title" className="mb-4 text-center text-base font-bold text-surface-50">
          Pick a Category
        </h3>

        <div className="grid grid-cols-3 gap-2 sm:grid-cols-4" role="listbox" aria-label="Categories">
          {cats.map((cat, i) => (
            <motion.button
              key={cat.id}
              type="button"
